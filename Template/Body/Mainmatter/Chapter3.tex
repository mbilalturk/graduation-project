...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[>=Latex, font=\small,
        box/.style={draw, rounded corners, minimum height=12mm, minimum width=28mm, align=center},
        io/.style={draw, fill=gray!12, minimum height=12mm, minimum width=28mm, align=center}]
        % sequence branch (top)
        \node[io]  (seqin)   at (0,0)    {Sequence input\\(window of recent\\segments)};
        \node[box] (lstm)    at (4.2,0)  {2-layer LSTM\\(128 units)};
        % context branch (bottom)
        \node[io]  (ctxin)   at (0,-2.6) {Context input\\(static features)};
        \node[box] (dense32) at (4.2,-2.6) {Dense\\(32, ReLU)};
        % merge and head (middle)
        \node[box] (concat)  at (8.4,-1.3)  {Concatenate};
        \node[box] (dense64) at (12.0,-1.3) {Dense\\(64, ReLU)};
        \node[io]  (out)     at (15.6,-1.3) {Travel-time\\prediction};
        % arrows
        \draw[->] (seqin)   -- (lstm);
        \draw[->] (ctxin)   -- (dense32);
        \draw[->] (lstm)    -- (concat);
    ...
    \caption{\label{fig:lstm}Structure of the dual-input LSTM. The sequence branch processes a sliding window of recent segments with a two-layer LSTM, while the context branch processes the static features of the target segment through a dense layer. Their outputs are concatenated and passed through a fully connected layer to produce the travel-time prediction.}
\end{figure}
