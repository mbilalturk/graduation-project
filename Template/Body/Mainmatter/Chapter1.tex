...
The work is anchored on a real, self-collected dataset. A custom data collector continuously queried the İzmir open transit Application Programming Interface (API) for 27 days, recording approximately two million GPS observations across three bus routes. From these observations, clean stop-to-stop travel-time segments were extracted and enriched with engineered features. A range of models was then trained and compared, from simple baselines (a naïve schedule-based predictor and a historical average) to classical machine learning models (Random Forest and XGBoost) and a deep learning model (a dual-input Long Short-Term Memory network, LSTM). The methodology is reproducible end to end and every reported result is fixed by a random seed.
...
\begin{itemize}
    ...
    \item \textbf{Weather coverage:} only clear and cloudy conditions were observed during the collection window, so the effect of rain or snow could not be demonstrated.
